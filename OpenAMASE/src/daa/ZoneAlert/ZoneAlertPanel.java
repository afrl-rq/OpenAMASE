...
public class ZoneAlertPanel extends JPanel {
    MiniAlert alert = new MiniAlert();
    ...
    public ZoneAlertPanel(AirVehicleConfiguration avc, Color color){
        setLayout(new BorderLayout(40, 5));
        ...
        totalPanel.add(textArea, BorderLayout.WEST);
        totalPanel.add(alertPanel, BorderLayout.EAST);
    ...
    public void update(AirVehicleState avs) {
        ...
        long id = avs.getID();
        if (AircraftColors.getNewColor(id) == Color.RED) {
            alert.setColor(Color.RED);
        } else if (AircraftColors.getNewColor(id) == Color.YELLOW){
            alert.setColor(Color.YELLOW);
        } else{
