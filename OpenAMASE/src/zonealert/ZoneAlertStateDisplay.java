...
//A copy of VehicleStateDisplay that swaps out VehiclePanel for ZoneAlertPanel.
 ...
 * @author AFRL/RQQD
 */
public class ZoneAlertStateDisplay extends AmasePlugin {
    
    private AppEventManager dataMgr = null;
    HashMap<Long,ZoneAlertPanel> vehicleMap = new HashMap<>();
    JPanel statusListPanel = new JPanel();
    JScrollPane scrollpane;
    
    public ZoneAlertStateDisplay() {
    ...
    public void eventOccurred(Object evt) {
        ...
        if (evt instanceof AirVehicleConfiguration) {
            ...
            ZoneAlertPanel vPanel = vehicleMap.get(avc.getID());
            if (vPanel == null) {
                vPanel = new ZoneAlertPanel(avc, AircraftColors.getColor(avc.getID()));
            ...
            ZoneAlertPanel vPanel = vehicleMap.get(avs.getID());
            if (vPanel != null) {
                vPanel.update(avs);
            }
        }
