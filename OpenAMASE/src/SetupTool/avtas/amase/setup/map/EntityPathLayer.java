 ...
 * @author AFRL/RQQD
 */
public class EntityPathLayer extends GraphicsLayer<MapGraphic> implements AppEventListener,
    ...
    MapGraphicsList<MapGraphicsList> missionList = new MapGraphicsList<>();
    ...
    public void eventOccurred(Object event) {
        if (event instanceof EntityConfiguration) {
            EntityConfiguration avc = (EntityConfiguration) event;
            if (!entityIdList.contains(avc.getID())) {
                entityIdList.add(avc.getID());
            }
        }

        else if (event instanceof FollowPathCommand) {
            FollowPathCommand mc = (FollowPathCommand) event;
            ...
            SelectObjectEvent selObjEvent = (SelectObjectEvent) event;
            for (MapGraphic g : missionList) {
                if (g.getRefObject() == selObjEvent.getObject()) {
                    g.setPainter(activeColor, 1);
                }
                else {
                    g.setPainter(inactiveColor, 1);
                }
            }
        }
    }
    ...
    protected MapGraphicsList drawAction(VehicleActionCommand vc) {
        ...
        missionGraphic.setPainter(inactiveColor, 1);
        refresh();
        return missionGraphic;
    }

    protected MapGraphicsList drawRoute(FollowPathCommand mc) {

        MapGraphicsList<MapGraphic> missionGraphic = new MapGraphicsList();
        missionGraphic.setRefObject(mc);

        for (int i = 0; i < mc.getWaypointList().size(); i++) {
            PathWaypoint wp = mc.getWaypointList().get(i);
        ...
        // draw waypoint line
        Waypoint wp = getWaypoint(mc.getWaypointList(), mc.getFirstWaypoint());
        int numWps = mc.getWaypointList().size();
        int i = 0;
        if (wp != null) {
            MapPoly polyGraphic = new MapPoly();
            polyGraphic.addPoint(wp.getLatitude(), wp.getLongitude());
            polyGraphic.setPainter(inactiveColor, new BasicStroke(2f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND));
            missionGraphic.add(0, polyGraphic);
            polyGraphic.setRefObject(mc);

            while (i < numWps) {
                Waypoint nextWp = getWaypoint(mc.getWaypointList(), wp.getNextWaypoint());
    ...
    static Waypoint getWaypoint(List<? extends Waypoint> list, long wpNum) {
    ...
    public void mousePressed(MouseEvent e, double lat, double lon) {
        ...
        dragCommand = getAssociatedObject(dragGraphic, FollowPathCommand.class);
    }
    ...
    public void addPopupMenuItems(JPopupMenu menu, MouseEvent e, final double lat, final double lon) {
        ...
        JMenu addWpMenu = new JMenu("Add Path Waypoint");
        menu.add(addWpMenu);

        for (Long id : entityIdList) {

            FollowPathCommand tmp_cmd = null;
            for (MapGraphic g : missionList) {
                if (g.getRefObject() instanceof FollowPathCommand) {
                    tmp_cmd = (FollowPathCommand) g.getRefObject();
            ...
            if (tmp_cmd == null) {
                tmp_cmd = new FollowPathCommand();
                tmp_cmd.setVehicleID(id);
            }

            final FollowPathCommand mc = tmp_cmd;

            JMenuItem wpMenu = new JMenuItem("to Entity " + mc.getVehicleID());
            wpMenu.addActionListener(new ActionListener() {

                @Override
                public void actionPerformed(ActionEvent e) {
                    PathWaypoint wp = (PathWaypoint) getLastWaypoint(mc);
                    PathWaypoint newWp;
                    if (wp != null) {
                        newWp = wp.clone();
                    } else {
                        newWp = new PathWaypoint();
                    }
        ...
        if (g != null) {
            ...
            if (loc instanceof PathWaypoint) {

                final PathWaypoint wp = (PathWaypoint) loc;
                final FollowPathCommand mc = (FollowPathCommand) g.getParent().getRefObject();
                ...
                final JMenuItem editMissionItem = new JMenuItem("Edit Path Command " + mc.getVehicleID());
                editMissionItem.addActionListener(new ActionListener() {

                    @Override
                    public void actionPerformed(ActionEvent e) {
                        Object newMC = ObjectTree.showEditWindow(mc.clone(), editMissionItem, "Edit Path Command " + mc.getVehicleID());
                ...
                final Object command = getAssociatedObject(g, FollowPathCommand.class);
    ...
    protected static Waypoint getLastWaypoint(FollowPathCommand mc) {
        int numTraversed = 0;
        Waypoint wp = getWaypoint(mc.getWaypointList(), mc.getFirstWaypoint());
        if (wp == null) {
            return null;
        }
        while (wp.getNextWaypoint() != 0 && numTraversed < mc.getWaypointList().size()) {
            numTraversed++;
            wp = getWaypoint(mc.getWaypointList(), wp.getNextWaypoint());
        }
        return wp;
    }

    protected static ArrayList<Waypoint> getByNextWaypoint(FollowPathCommand mc, long nextWpNum) {
        ArrayList<Waypoint> retList = new ArrayList<>();
        for (Waypoint wp : mc.getWaypointList()) {
     ...
     * @param mc
     */
    protected static long getUniqueWaypointNumber(FollowPathCommand mc) {
        long val = 1;
        while (getWaypoint(mc.getWaypointList(), val) != null) {
            val++;
        }
        return val;
    }
